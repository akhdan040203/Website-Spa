...
const homeArticleSection = document.querySelector("#artikel");
const homeArticles = homeArticleSection?.querySelector("[data-home-articles]");
if (homeArticleSection && homeArticles) {
  let articleRequest = null;

  const loadArticlePage = async (url, updateHistory = true) => {
    articleRequest?.abort();
    articleRequest = new AbortController();
    homeArticles.classList.add("is-loading");
    homeArticles.setAttribute("aria-busy", "true");

    try {
      const response = await fetch(url, {
        headers: { "X-Requested-With": "XMLHttpRequest" },
        signal: articleRequest.signal,
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);

      const page = new DOMParser().parseFromString(await response.text(), "text/html");
      const incoming = page.querySelector("[data-home-articles]");
      if (!incoming) throw new Error("Konten artikel tidak ditemukan");

      homeArticles.innerHTML = incoming.innerHTML;
      if (updateHistory) history.pushState({ homeArticles: true }, "", url);
      AOS.refreshHard();
    } catch (error) {
      if (error.name !== "AbortError") window.location.assign(url);
    } finally {
      homeArticles.classList.remove("is-loading");
      homeArticles.setAttribute("aria-busy", "false");
    }
  };

  homeArticleSection.addEventListener("click", (event) => {
    const link = event.target.closest(".home-article-pagination a");
    if (!link) return;
    event.preventDefault();
    loadArticlePage(link.href);
  });

  window.addEventListener("popstate", () => {
    loadArticlePage(window.location.href, false);
  });
}
